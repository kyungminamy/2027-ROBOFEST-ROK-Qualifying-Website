...
export const competition = {
  ...
  registration: {
     ...
     *
     *  ★ 이 폼은 4개 섹션(페이지)으로 나뉘어 있습니다. ★
     *    섹션마다 길이가 크게 달라서, 하나의 높이로는 완벽히 맞출 수 없습니다.
     *
     *  2026-07-30 실측값 (휴대폰 390px 화면 기준):
     *    섹션1 (신청학교, 2문항)     1,639px
     *    섹션2 (코치 인적사항, 10문항) 2,410px
     *    섹션3 (팀원 인적사항, 21문항) 3,741px  ← 가장 김
     *    섹션4 (기타사항, 1문항)        900px  ← 가장 짧음
     *
     *  현재 값 3855 는 '가장 긴 섹션3'에 맞춘 값입니다.
     *  → 어느 섹션에서도 폼 안쪽 스크롤바가 생기지 않습니다. (좋음)
     *  → 대신 짧은 섹션에서는 아래에 빈 여백이 크게 남습니다.
     *     특히 섹션4에서는 약 3,000px(휴대폰 7화면 분량)이 빕니다. (나쁨)
     *
     *  【 여백을 없애고 싶다면 】
     *   구글폼에서 '섹션(페이지) 나누기' 3개를 없애 한 페이지로 만드세요.
     *   그러면 길이가 하나로 정해져서 여백 없이 딱 맞습니다.
     *   (그 뒤 아래 숫자를 새로 실측한 값으로 바꾸세요)
     *
     *  【 스크롤바가 생겼을 때 】
     *   폼에 질문을 추가하면 섹션3이 더 길어집니다. 숫자를 키우세요.
     *
     *  ★ 애매하면 큰 쪽으로 두세요. ★
    ...
    /**
     * 개인정보 국외이전 안내 문구 — 폼 위에 항상 표시됩니다
     *
     *  ⚠️ 법적으로 필요한 안내입니다. 지우지 마세요.
     *     구글폼은 해외 서버에 정보를 저장하므로, 신청자가 입력을
     *     시작하기 전에 이 사실을 알려야 합니다.
     *
     *  담당자가 문구를 다듬을 수 있도록 여기에 두었습니다.
     *  법적 표현은 담당자 검토를 받으세요.
     */
    privacyNotice:
      '참가 신청은 구글폼(Google Forms)으로 접수됩니다. 입력하신 개인정보는 구글의 해외 서버에 저장되며, 대회 운영 목적으로만 사용됩니다. 만 14세 미만 학생은 법정대리인(보호자)의 동의가 필요합니다. 자세한 내용은 폼의 첫 번째 동의 항목에서 확인하실 수 있습니다.',
