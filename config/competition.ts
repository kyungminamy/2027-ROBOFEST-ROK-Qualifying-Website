...
export const competition = {
   ...
   *  첫 화면 배경 사진
   *
   *  ★ 사진을 바꾸려면 아래 두 줄의 파일 이름만 바꾸세요. ★
   *    큰 화면용(wide)과 휴대폰용(small) 두 가지를 씁니다.
   *    휴대폰용을 따로 두는 이유: 큰 사진을 휴대폰으로 받으면 데이터가
   *    많이 들고 느립니다. 지도교사·학부모 대부분이 휴대폰으로 봅니다.
   *
   *  ★ 둘 다 비워두면('') 사진 없이 남색 격자무늬가 나옵니다. ★
   *    (파일 경로가 틀려도 깨진 사진이 뜨지 않게 하기 위한 안전장치)
   *
   *  【 새 사진으로 바꾸는 방법 】
   *   1. 원본 사진을 준비합니다 (가로가 긴 사진이 잘 맞습니다)
   *   2. 가로 2000px / 900px 두 가지로 줄여서 public/hero/ 에 올립니다
   *      → 파일 크기는 각각 400KB / 150KB 이하를 권장합니다
   *   3. 아래 두 줄을 새 파일 이름으로 바꿉니다
   *
   *  ⚠️ 사진 위에 흰 글자가 올라갑니다. 글자가 잘 안 보이면
   *     globals.css 의 .hero-photo 에서 어두운 막(overlay) 농도를 올리세요.
   *
   *  ⚠️ 지금 사진은 미국 LTU에서 열린 ROBOFEST 세계대회 사진입니다.
   *     부산 국내예선 사진이 아닙니다. 그래서 화면에 그 사실을 밝히는
   *     작은 글씨(heroImageCredit)를 함께 넣었습니다. 사진을 바꾸면
   *     그 글씨도 같이 고치거나, 우리 대회 사진이면 ''로 비우세요.
   * ========================================================================== */
  heroImage: {
    /** 태블릿·컴퓨터용 (가로 2000px) */
     ...
     *
     *  첫 화면 칸은 사진보다 가로로 훨씬 넓어서, 사진의 위아래가 잘립니다.
     *  그래서 '어디를 남길지'를 정해 줘야 합니다.
     *
     *  쓰는 법: '가로 세로' 순서로 적습니다.
     *    가로 — left(왼쪽) / center(가운데) / right(오른쪽) 또는 0%~100%
     *    세로 — top(위) / center(가운데) / bottom(아래) 또는 0%~100%
     *
     *  예) 'center bottom' = 좌우는 가운데, 위아래는 아래쪽을 보여줌
     *      '70% bottom'    = 좌우는 오른쪽에 가깝게, 아래쪽을 보여줌
     *
     *  ★ 사진을 바꾸면 이 값도 다시 맞춰야 합니다. ★
     *    보여주고 싶은 사람·물건이 화면 밖으로 잘려 나가기 때문입니다.
     *  ---------------------------------------------------------------- */

    /** 태블릿·컴퓨터에서 보여줄 위치 */
    positionWide: 'center bottom',

    /** 휴대폰에서 보여줄 위치 (좁아서 좌우도 잘립니다) */
    positionSmall: '68% bottom',
  },

  /** 사진 출처 안내. 비우면('') 표시되지 않습니다. */
  heroImageCredit: 'ROBOFEST 세계대회 (미국 Lawrence Technological University)',
